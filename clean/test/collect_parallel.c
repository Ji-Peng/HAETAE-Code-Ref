 ...
 *   Header: magic(u32='HHP1') N K L M LN num_sigs rec_i32 pk_bytes
 *   Public key: pk_bytes raw bytes
 ...
 *
 * Usage: ./collect_hint_pk2 <num_sigs> <output_file>
...
/* See avx2/test/collect_parallel.c for rationale.  accept-all pack probe. */
...
/* Streamlined parallel collector for the pilot-weighted passive attack.
 *
 *   collect_parallel keygen <keyfile>
 *       generate ONE shared key; write <keyfile> (pk||sk) and <keyfile>.meta
 *       (N K L M pkbytes, pk, s1, s2) for scoring.
 *   collect_parallel run <num> <offset> <keyfile> <out>
 *       load the shared key; sign messages [offset, offset+num); stream a
 *       minimal record per signature to <out> ("-" = stdout):
 *           z1 (L*N i32) | z2v=pub_z2 (K*N i32) | c (N i32) | bdiag (1 i32)
 *       so many processes can share one key over disjoint message ranges.
 */
int main(int argc, char *argv[])
{
    if (argc >= 3 && strcmp(argv[1], "keygen") == 0) {
        ...
        if (!kf) { fprintf(stderr, "cannot write keyfile %s\n", keyfile); return 1; }
        fwrite(pk, 1, HAETAE_CRYPTO_PUBLICKEYBYTES, kf);
        fwrite(sk, 1, HAETAE_CRYPTO_SECRETKEYBYTES, kf);
        fclose(kf);
        ...
        char metafile[2048];
        snprintf(metafile, sizeof(metafile), "%s.meta", keyfile);
        FILE *mf = fopen(metafile, "wb");
        if (!mf) { fprintf(stderr, "cannot write meta\n"); return 1; }
        uint32_t mh[5] = {(uint32_t)N, (uint32_t)K, (uint32_t)L, (uint32_t)M,
                          (uint32_t)HAETAE_CRYPTO_PUBLICKEYBYTES};
        fwrite(mh, sizeof(uint32_t), 5, mf);
        fwrite(pk, 1, HAETAE_CRYPTO_PUBLICKEYBYTES, mf);
        for (unsigned int i = 0; i < M; i++)
            fwrite(s1.vec[i].coeffs, sizeof(int32_t), N, mf);
        for (unsigned int i = 0; i < K; i++)
            fwrite(s2.vec[i].coeffs, sizeof(int32_t), N, mf);
        fclose(mf);
        fprintf(stderr, "keygen -> %s (+ .meta) N=%d K=%d L=%d M=%d\n",
                keyfile, N, K, L, M);
        return 0;
    }

    if (argc >= 6 && strcmp(argv[1], "run") == 0) {
        uint64_t num_sigs = strtoull(argv[2], NULL, 10);
        ...
        if (!kf) { fprintf(stderr, "cannot read keyfile %s\n", keyfile); return 1; }
        if (fread(pk, 1, HAETAE_CRYPTO_PUBLICKEYBYTES, kf) != HAETAE_CRYPTO_PUBLICKEYBYTES ||
            fread(sk, 1, HAETAE_CRYPTO_SECRETKEYBYTES, kf) != HAETAE_CRYPTO_SECRETKEYBYTES) {
            fprintf(stderr, "keyfile too short\n"); fclose(kf); return 1;
        }
        fclose(kf);
        ...
        for (uint64_t t = 0; t < num_sigs; t++) {
            uint64_t m = t + offset;       /* 8-byte message: 2^64 distinct, NEVER wraps */
            memcpy(msg, &m, sizeof(m));     /* low 8 bytes = little-endian m */
            ...
            if ((t + 1) % 100000 == 0) {
                ...
                fprintf(stderr, "  off=%llu [%llu/%llu] %.0f sigs/sec\n",
                        (unsigned long long)offset, (unsigned long long)(t + 1),
                        (unsigned long long)num_sigs, (t + 1) / el);
            }
        }
