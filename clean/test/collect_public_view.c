/**
 * collect_public_view.c
 *
 * Collects HAETAE signatures as seen by the public verifier.  The old
 * collect_signatures.c exports internal fixed-point y,z,b for diagnostics;
 * this collector instead exports rounded signer responses and
 * verifier-reconstructed public responses, so public-only analyses can
 * check whether their signal survives signature packing and verifier
 * reconstruction.
 *
 * Binary output format:
 *   Header: magic(u32='HPV1') N K L M LN num_sigs rec_i32
 ...
 *     verifier z1  : L polynomials (composed from signature high/low bits)
 *     verifier z2  : K polynomials (reconstructed \tilde{z}_2)
 *     challenge c  : N coefficients
 *
 * Usage: ./collect_public2 <num_sigs> <output_file>
...
#define PUBLIC_VIEW_MAGIC 0x31565048u

static void reconstruct_public_response(polyvecl *out_z1, polyveck *out_z2,
                                        const uint8_t *pk, const poly *c,
...
static int sign_and_collect_public(polyvecl *out_z1rnd,
                                   polyveck *out_z2rnd,
                                   polyvecl *out_pub_z1,
                                   polyveck *out_pub_z2, poly *out_c,
...
reject:
    ...
    *out_z2rnd = z2rnd_out;
    *out_c = c;
    reconstruct_public_response(out_pub_z1, out_pub_z2, pk, &c, &lb_z1,
                                &hb_z1, &h);
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "HAETAE-%d public-view signature collection\n",
            HAETAE_MODE);
    ...
    uint32_t rec_i32 = (uint32_t)((2 * (L + K) + 1) * N);
    uint32_t header[8] = {PUBLIC_VIEW_MAGIC, N,      K, L, M, LN,
                          num_sigs,          rec_i32};
    ...
    for (uint32_t t = 0; t < num_sigs; t++) {
        ...
        sign_and_collect_public(&z1rnd, &z2rnd, &pub_z1, &pub_z2, &c, msg,
                                sizeof(msg), pk, sk);
        ...
        for (unsigned int i = 0; i < L; i++) {
            fwrite(pub_z1.vec[i].coeffs, sizeof(int32_t), N, fp);
        }
        for (unsigned int i = 0; i < K; i++) {
