...
static const union {
    ...
    .arr = {3266,  6520,  3266,  6520,  3266,  6520,  3266,  6520,  9748,
            12938, 9748,  12938, 9748,  12938, 9748,  12938, 16079, 19159,
            16079, 19159, 16079, 19159, 16079, 19159, 22168, 25096, 22168,
            25096, 22168, 25096, 22168, 25096, 27934, 30674, 27934, 30674,
            27934, 30674, 27934, 30674, 33309, 35833, 33309, 35833, 33309,
            35833, 33309, 35833, 38241, 40531, 38241, 40531, 38241, 40531,
            38241, 40531, 42698, 44742, 42698, 44742, 42698, 44742, 42698,
            44742, 46663, 48460, 46663, 48460, 46663, 48460, 46663, 48460,
            50135, 51690, 50135, 51690, 50135, 51690, 50135, 51690, 53128,
            54454, 53128, 54454, 53128, 54454, 53128, 54454, 55670, 56781,
            55670, 56781, 55670, 56781, 55670, 56781, 57794, 58712, 57794,
            58712, 57794, 58712, 57794, 58712, 59541, 60287, 59541, 60287,
            59541, 60287, 59541, 60287, 60956, 61554, 60956, 61554, 60956,
            61554, 60956, 61554, 62085, 62556, 62085, 62556, 62085, 62556,
            62085, 62556, 62972, 63337, 62972, 63337, 62972, 63337, 62972,
            63337, 63657, 63936, 63657, 63936, 63657, 63936, 63657, 63936,
            64178, 64388, 64178, 64388, 64178, 64388, 64178, 64388, 64569,
            64724, 64569, 64724, 64569, 64724, 64569, 64724, 64857, 64970,
            64857, 64970, 64857, 64970, 64857, 64970, 65066, 65148, 65066,
            65148, 65066, 65148, 65066, 65148, 65216, 65273, 65216, 65273,
            65216, 65273, 65216, 65273, 65321, 65361, 65321, 65361, 65321,
            65361, 65321, 65361, 65394, 65422, 65394, 65422, 65394, 65422,
            65394, 65422, 65444, 65463, 65444, 65463, 65444, 65463, 65444,
            65463, 65478, 65490, 65478, 65490, 65478, 65490, 65478, 65490,
            65500, 65508, 65500, 65508, 65500, 65508, 65500, 65508, 65514,
            65519, 65514, 65519, 65514, 65519, 65514, 65519, 65523, 65527,
...
static const union {
    __m256i vec[2];
    uint16_t arr[32];
} mask_avx = {.arr = {0xffff, 0, 0,      0, 0xffff, 0, 0,      0,
...
static const union {
    ...
    .arr = {(1ULL << 48) - 1, (1ULL << 48) - 1, (1ULL << 48) - 1,
            (1ULL << 48) - 1, (1ULL << 24) - 1, (1ULL << 24) - 1,
            (1ULL << 24) - 1, (1ULL << 24) - 1, (1ULL << 16) - 1,
            (1ULL << 16) - 1, (1ULL << 16) - 1, (1ULL << 16) - 1}};
...
static void sample_gauss16_x4(__m256i *r)
{
    ...
    for (i = 0; i < NUM_GAUSSIANS; i++) {
        ...
        // only select the lower 32 bit of each 64-bit value (remove
        // redundancy)
...
static void sample_candidates_x4(__m256i *sc, __m256i *sqr, __m256i *rej)
{
    ...
    for (i = 0; i < NUM_GAUSSIANS; i++) {
        ...
        // = a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +  2 *
        // 2^48a[31:0]a[78:48]  +  2^64a[47:32]^2  +  2 *
        // 2^80a[47:32]a[78:48]  +  2^96a[78:48]^2
        ...
        reg[3] = _mm256_add_epi64(
            reg[3],
            reg[4]);  // (a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]) >> 33
        // reg[3] = _mm256_add_epi64(reg[3], reg[13]); // add 1<<14
        reg[3] = _mm256_srli_epi64(
            reg[3], 16);  // (a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]) >> 49
        reg[3] = _mm256_add_epi64(
            reg[3], reg[9]);  // (a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +
                              // 2 * 2^48a[31:0]a[78:48]) >> 49
        reg[3] = _mm256_srli_epi64(reg[3], 15);
        reg[3] = _mm256_add_epi64(
            reg[3],
            reg[10]);  // (a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +  2 *
                       // 2^48a[31:0]a[78:48]  +  2^64a[47:32]^2) >> 64
        // reg[3] = _mm256_add_epi64(reg[3], reg[14]); // add 1<<11
        reg[3] = _mm256_srli_epi64(
            reg[3], 12);  // (a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +  2 *
                          // 2^48a[31:0]a[78:48]  +  2^64a[47:32]^2) >> 76
        reg[6] = _mm256_slli_epi64(reg[11], 5);   // a[47:32]a[78:48] << 5
        reg[9] = _mm256_slli_epi64(reg[12], 20);  // a[78:48]^2 << 20
        reg[4] = _mm256_srli_epi64(
            reg[12],
            28);  // a[78:48]^2 >> 28 --- the 36 high bits of reg[12]
        reg[9] = _mm256_and_si256(
            reg[9], reg[7]);  // (a[78:48]^2 << 20) & ((1UL<<48)-1) --- the
                              // 28 low bits of reg[12]

        reg[3] = _mm256_add_epi64(
            reg[6],
            reg[3]);  // LOW[(a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +  2 *
                      // 2^48a[31:0]a[78:48]  +  2^64a[47:32]^2  +  2 *
                      // 2^80a[47:32]a[78:48]) >> 76]
        reg[3] = _mm256_add_epi64(
            reg[9],
            reg[3]);  // LOW[(a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +  2 *
                      // 2^48a[31:0]a[78:48]  +  2^64a[47:32]^2  +  2 *
                      // 2^80a[47:32]a[78:48]  +  2^96a[78:48]^2) >> 76]
        reg[6] = _mm256_srli_epi64(reg[3], 48);  // copy upper bits
        reg[4] = _mm256_add_epi64(
            reg[4],
            reg[6]);  // HIGH[(a[31:0]^2  +  2 * 2^32a[31:0]a[47:32]  +  2
                      // * 2^48a[31:0]a[78:48]  +  2^64a[47:32]^2  +  2 *
                      // 2^80a[47:32]a[78:48]  +  2^96a[78:48]^2) >> 76]
        reg[3] =
            _mm256_and_si256(reg[3], reg[7]);  // mask with (1ULL<<48)-1
        ...
        // reg[5]
        reg[0] = _mm256_mullo_epi16(
            reg[0], reg[0]);  // this is faster, and the sc value is <= 64
...
static inline void smulh48_avx(__m256i *a, const __m256i *b, __m256i *ah,
                               __m256i *bh, __m256i *al, __m256i *bl,
                               __m256i *tmp, __m256i *tmp2,
                               const __m256i *mask24, const __m256i *one23)
{
    *ah = _mm256_srli_epi64(*a, 24);
    *tmp = _mm256_srli_epi64(*ah, 24);  // obtain 16 sign bits
    *tmp2 = _mm256_slli_epi64(
        *tmp, 40);  // shift up, MSByte has still no sign bits
    ...
    *bl = _mm256_srli_epi64(
        *a, 24);  // now we only have 16 sign bits left as LSBs
    *al = _mm256_slli_epi64(
        *bl,
        40);  // shift sign bits up, but the MSByte still has no sign bits
    *bl = _mm256_slli_epi64(
        *bl, 48);  // shift sign bits up, including the MSByte
    *a = _mm256_or_si256(*a, *al);  // set sign bits
    *a = _mm256_or_si256(*a, *bl);  // set sign bits
    *a = _mm256_add_epi64(*a, *tmp);

    *a = _mm256_srli_epi64(*a, 24);
    *tmp = _mm256_srli_epi64(
        *a, 32);  // now we only have 8 sign bits left as LSBs
...
static const union {
    __m256i vec[9];
    int64_t arr[9 * 4];
} exp_avx = {.arr = {-0x0000B6C6340925AELL,
                     -0x0000B6C6340925AELL,
                     -0x0000B6C6340925AELL,
                     -0x0000B6C6340925AELL,
...
static void approx_exp_4x(__m256i *rej)
{
    ...
    for (i = 0; i < NUM_GAUSSIANS; i++) {
        ...
        smulh48_avx(&reg[0], &reg[4], &reg[5], &reg[6], &reg[7], &reg[8],
                    &reg[9], &reg[10], &reg[2], &reg[3]);
...
static const union {
    __m256i vec;
    uint64_t arr[4];
} nolsb_avx = {.arr = {~((uint64_t)1), ~((uint64_t)1), ~((uint64_t)1),
                       ~((uint64_t)1)}};

static inline void _rejcond(__m256i ioreg[3], __m256i reg[5],
                            const __m256i *nolsb, const __m256i *one,
                            const __m256i *zero)
{
    ...
    reg[3] = _mm256_add_epi64(
        reg[3], *one);  // reg[3] is zero when ioreg[2] is zero, else one
    reg[4] = _mm256_or_si256(reg[3], ioreg[0]);
    reg[2] = _mm256_and_si256(reg[2], reg[4]);
    reg[2] = _mm256_and_si256(reg[2], *one);  // extract only lsb
    ioreg[0] = _mm256_sub_epi64(
        *zero, reg[2]);  // all-ones if reg[2] was 0, else zero
}
...
static const union {
    __m256i vec;
    uint64_t arr[4];
} minusone_avx = {.arr = {-1, -1, -1, -1}};

static void sum_sqr_4x(uint64_t sqsum[2], const __m256i *sqr,
                       const __m256i *rej, __m256i *len)
{
    size_t i, j;
    __m256i reg[16];
    union {
    ...
    for (i = 0; i < NUM_GAUSSIANS; i += 2) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            reg[13] =
                _mm256_cmpgt_epi64(reg[15], reg[14]);  // compare if >= 0
            reg[j + 2] = _mm256_and_si256(reg[j + 2],
                                          reg[13]);  // set to zero if < 0
...
// static void move_to_mem_4x(r0, r1, r2, r3, &ctr0, &ctr1, &ctr2, &ctr3,
// sample_candidates.vec, rejection.vec);
static void move_to_mem_4x(uint64_t *r0, uint64_t *r1, uint64_t *r2,
                           uint64_t *r3, const size_t len0,
                           const size_t len1, const size_t len2,
                           const size_t len3, const __m256i *sc,
                           const __m256i *rej)
{
    size_t i;
    uint64_t accepted[4];
    ...
    for (i = 0; i < NUM_GAUSSIANS; i++) {
        ...
        _mm_storel_pd(
            (__attribute__((__may_alias__)) double *)&accepted[0], t);
        _mm_storeh_pd(
            (__attribute__((__may_alias__)) double *)&accepted[1], t);
        t = _mm_castsi128_pd(_mm256_extracti128_si256(rej[i], 1));
        _mm_storel_pd(
            (__attribute__((__may_alias__)) double *)&accepted[2], t);
        _mm_storeh_pd(
            (__attribute__((__may_alias__)) double *)&accepted[3], t);

        t = _mm_castsi128_pd(_mm256_castsi256_si128(sc[i]));
        if (ctr[0] < len0) {
            _mm_storel_pd(
                (__attribute__((__may_alias__)) double *)&r0[ctr[0]], t);
            ctr[0] += accepted[0] & 1;
        }
        if (ctr[1] < len1) {
            _mm_storeh_pd(
                (__attribute__((__may_alias__)) double *)&r1[ctr[1]], t);
            ctr[1] += accepted[1] & 1;
        }
        t = _mm_castsi128_pd(_mm256_extracti128_si256(sc[i], 1));
        if (ctr[2] < len2) {
            _mm_storel_pd(
                (__attribute__((__may_alias__)) double *)&r2[ctr[2]], t);
            ctr[2] += accepted[2] & 1;
        }
        if (ctr[3] < len3) {
            _mm_storeh_pd(
                (__attribute__((__may_alias__)) double *)&r3[ctr[3]], t);
            ctr[3] += accepted[3] & 1;
        }
    }
}

void sample_gauss_N_4x(uint64_t *r0, uint64_t *r1, uint64_t *r2,
                       uint64_t *r3, uint8_t *signs0, uint8_t *signs1,
                       uint8_t *signs2, uint8_t *signs3, fp96_76 *sqsum,
                       const uint8_t seed[CRHBYTES], const uint16_t nonce0,
                       const uint16_t nonce1, const uint16_t nonce2,
                       const uint16_t nonce3, const size_t len0,
                       const size_t len1, const size_t len2,
                       const size_t len3)
{
    ...
    ALIGNED_UINT8(POLY_HYPERBALL_NBLOCKS_4X * STREAM256_BLOCKBYTES * 4)
    outbuf;
    ...
    shake256x4_absorb_once(&state, buf[0].coeffs, buf[1].coeffs,
                           buf[2].coeffs, buf[3].coeffs, CRHBYTES + 2);
    shake256x4_squeezeblocks_vec(&outbuf.vec[0], POLY_HYPERBALL_NBLOCKS_4X,
                                 &state);
#else
    for (size_t i = 0; i < 4; i++) {
        ...
        stream256_squeezeblocks(
            &outbuf.coeffs[POLY_HYPERBALL_NBLOCKS_4X * i],
            POLY_HYPERBALL_NBLOCKS_4X, &state[i]);  // TODO reorder bytes!
    }
#endif
    ...
    move_to_mem_4x(r0, r1, r2, r3, len0 - len0 % N, len1 - len1 % N,
                   len2 - len2 % N, len3 - len3 % N, sample_candidates.vec,
                   rejection.vec);
    ...
    bytecnt = POLY_HYPERBALL_NBLOCKS_4X * STREAM256_BLOCKBYTES - N / 8 -
              NUM_GAUSSIANS * GAUSS_RAND_BYTES;
    if (ctr0 < len0) {
        uint8_t *bptr =
            &outbuf.coeffs[bytecnt + NUM_GAUSSIANS * GAUSS_RAND_BYTES +
                           N / 8];
        for (i = 0; i < bytecnt; i++) {
...
#ifndef HAETAE_USE_AES
        shake256x4_squeezeblocks(&outbuf2[0].coeffs[bytecnt],
                                 &outbuf2[1].coeffs[bytecnt],
                                 &outbuf2[2].coeffs[bytecnt],
                                 &outbuf2[3].coeffs[bytecnt], 1, &state);
#else
        if (ctr0 < len0) {
            stream256_squeezeblocks(&outbuf2[0].coeffs[bytecnt], 1,
                                    &state[0]);
        }
        if (ctr1 < len1) {
            stream256_squeezeblocks(&outbuf2[1].coeffs[bytecnt], 1,
                                    &state[1]);
        }
        if (ctr2 < len2) {
            stream256_squeezeblocks(&outbuf2[2].coeffs[bytecnt], 1,
                                    &state[2]);
        }
        if (ctr3 < len3) {
            stream256_squeezeblocks(&outbuf2[3].coeffs[bytecnt], 1,
                                    &state[3]);
        }
#endif
        bytecnt += STREAM256_BLOCKBYTES;

        ctr0 += sample_gauss(r0 + ctr0, sqsum, outbuf2[0].coeffs, bytecnt,
                             len0 - ctr0, len0 % N);
        ctr1 += sample_gauss(r1 + ctr1, sqsum, outbuf2[1].coeffs, bytecnt,
                             len1 - ctr1, len1 % N);
        ctr2 += sample_gauss(r2 + ctr2, sqsum, outbuf2[2].coeffs, bytecnt,
                             len2 - ctr2, len2 % N);
        ctr3 += sample_gauss(r3 + ctr3, sqsum, outbuf2[3].coeffs, bytecnt,
                             len3 - ctr3, len3 % N);

        if (ctr0 < len0 || ctr1 < len1 || ctr2 < len2 || ctr3 < len3) {
            size_t offset =
                (bytecnt / GAUSS_RAND_BYTES) * GAUSS_RAND_BYTES;
