/*
 * speed_hyperball.c — HAETAE avx2: 球采样各组件耗时测试 (CPU cycles)
 *
 * 测试组件:
 *   1. sample_gauss16       — CDT 查表采样 (标量)
 *   2. approx_exp           — 近似指数 (标量)
 *   3. fixpoint_square      — 定点数平方
 *   4. fixpoint_newton_invsqrt — Newton 法求逆平方根
 *   5. fixpoint_mul_rnd13   — 坐标缩放乘法 (标量)
 *   6. sample_gauss_N       — 一批 N 个高斯样本 (单路 SHAKE-256)
 *   7. sample_gauss_N_4x    — 4 路并行高斯样本 (keccak4x)
 *   8. polyfixveclk_sqnorm2 — 范数检查 (AVX2)
 *   9. polyfixveclk_sample_hyperball — 完整球采样
 */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
int main(void)
{
    ...
    polyfixvecl y1;
    polyfixveck y2;
    uint64_t samples[N * 4];
    uint8_t signs[N * 4 / 8];
    fp96_76 sqsum, invsqrt;
    volatile uint64_t dummy;

    printf("HAETAE%d avx2 — 球采样组件级 speed test\n", HAETAE_MODE);
    printf("N=%d, K=%d, L=%d\n\n", N, K, L);

    /* 1. sample_gauss16 (CDT lookup, scalar) */
    for (i = 0; i < NTESTS; ++i) {
        t[i] = cpucycles();
    ...
    /* 6. sample_gauss_N (single lane, N+1 samples) */
    for (i = 0; i < NTESTS; ++i) {
        sqsum.limb48[0] = 0;
        sqsum.limb48[1] = 0;
        t[i] = cpucycles();
    ...
    /* 7. sample_gauss_N_4x (4-way parallel, 4 × N samples) */
    for (i = 0; i < NTESTS; ++i) {
        sqsum.limb48[0] = 0;
        sqsum.limb48[1] = 0;
        t[i] = cpucycles();
        sample_gauss_N_4x(
            &samples[0], &samples[N], &samples[2*N], &samples[3*N],
            &signs[0], &signs[N/8], &signs[2*N/8], &signs[3*N/8],
            &sqsum, seed,
            (uint16_t)(i*4), (uint16_t)(i*4+1), (uint16_t)(i*4+2), (uint16_t)(i*4+3),
            N+1, N+1, N, N);
    }
    t[NTESTS] = cpucycles();
    print_results("sample_gauss_N_4x (4×N):", t, NTESTS);

    /* 8. polyfixveclk_sqnorm2 (AVX2) */
    ...
    /* 9. polyfixveclk_sample_hyperball (full) */
