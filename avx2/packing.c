 ...
 *              - const polyveck *b: polynomial vector of length K containg
 * b
 *              - const uint8_t seed[]: seed for A'
 **************************************************/
void pack_pk(uint8_t pk[HAETAE_CRYPTO_PUBLICKEYBYTES], polyveck *b,
...
void unpack_pk(polyveck *b, uint8_t seed[SEEDBYTES],
               const uint8_t pk[HAETAE_CRYPTO_PUBLICKEYBYTES])
{
    unsigned int i;
 ...
 * (encoding starting at offset 1)
 *              - const polyveck *s1: polyveck pointer containing s1
 **************************************************/
void pack_sk(uint8_t sk[HAETAE_CRYPTO_SECRETKEYBYTES],
             const uint8_t pk[HAETAE_CRYPTO_PUBLICKEYBYTES],
             const polyvecm *s0, const polyveck *s1,
             const uint8_t key[SEEDBYTES])
{
    unsigned int i;
    memcpy(sk, pk, HAETAE_CRYPTO_PUBLICKEYBYTES);
 ...
 *              - const polyvecl *lowbits_z1: pointer to vector LowBits(z1)
 * of length L
 *              - const polyvecl *highbits_z1: pointer to vector
 * HighBits(z1) of length L
             ...
             const polyveck *h)
{
    ...
    // The size of the encoded h and HB(z1) does not always fit in one
    // byte, thus we output a one byte offset to a fixed baseline
 ...
 *              - polyvecl *lowbits_z1: pointer to output vector
 * LowBits(z1)
 *              - polyvecl *highbits_z1: pointer to output vector
 * HighBits(z1)
               ...
               polyveck *h, const uint8_t sig[HAETAE_CRYPTO_BYTES])
{
    ...
    if (HAETAE_CRYPTO_BYTES <
        (2 + L * N + SEEDBYTES + size_enc_h + size_enc_hb_z1))
        return 1;  // invalid size_enc_h and/or size_enc_hb_z1

    if (decode_hb_z1(&highbits_z1->vec[0].coeffs[0], sig,
                     size_enc_hb_z1)) {
    ...
    unsigned int t = HAETAE_CRYPTO_BYTES -
                     (SEEDBYTES + L * N + 2 + size_enc_hb_z1 + size_enc_h);
    for (i = 0; i < t; i++)
        if (sig[i] != 0)
